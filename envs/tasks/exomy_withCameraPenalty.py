...
class Exomy_withCameraPenalty(VecTask):
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        # Create camera penalty tensor:
        self.camera_penalty = torch.zeros(
    ...
    def _create_ground_plane(self):
        ...
        heightfield[0:num_rows, :] =              wave_terrain(new_sub_terrain(), num_waves=0., amplitude=0.).height_field_raw
        heightfield[num_rows:2*num_rows, :] =     wave_terrain(new_sub_terrain(), num_waves=0., amplitude=0.).height_field_raw
        heightfield[2*num_rows:3*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[3*num_rows:4*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[4*num_rows:5*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[5*num_rows:6*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[6*num_rows:7*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[7*num_rows:8*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[8*num_rows:9*num_rows, :] =   discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[9*num_rows:10*num_rows, :] =  discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[10*num_rows:11*num_rows, :] = discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[11*num_rows:12*num_rows, :] = discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[12*num_rows:13*num_rows, :] = discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
        heightfield[13*num_rows:14*num_rows, :] = discrete_obstacles_terrain(new_sub_terrain(), max_height=1.0, min_size=0.4, max_size=0.6, num_rects=70).height_field_raw
    ...
    def set_targets(self, env_ids):
        ...
        self.target_root_positions[env_ids, 2] = 0.6
    ...
    def tensor_quat_to_eul(self):
        # https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
        # Quaternions format: X, Y, Z, W
        # Quat index:         0, 1, 2, 3
        quats = self.root_quats
    ...
    def post_physics_step(self):
        ...
        # self.root_eulertest = torch.from_numpy(root_quat.as_euler('xyz')).to(self.device)
        # print(self.root_eulertest[0])
        ...
        # Penalty for getting too close to objects (flat terrain: cam_5_min = 0.36, cam_4_min = 0.42, cam_3_min = 0.52, cam_1_min = 0.94)
        abs_cam_obs_4 = abs(self.obs_buf[:,35:45])
        cam_buf_min_4 = torch.min(abs_cam_obs_4, dim=1)
        cam_4 = torch.where(((cam_buf_min_4[0] < 0.33) | (cam_buf_min_4[0] > 0.55)), 1, 0)
        cam_4_penalty = 1 / (1 + cam_buf_min_4[0] * cam_buf_min_4[0]) * cam_4

        abs_cam_obs_3 = abs(self.obs_buf[:,25:35])
        cam_buf_min_3 = torch.min(abs_cam_obs_3, dim=1)
        cam_3 = torch.where(((cam_buf_min_3[0] < 0.47) | (cam_buf_min_3[0] > 0.7)), 1, 0)
        cam_3_penalty = 1 / (1 + cam_buf_min_3[0] * cam_buf_min_3[0]) * cam_3

        abs_cam_obs_1 = abs(self.obs_buf[:,5:15])
        cam_buf_min_1 = torch.min(abs_cam_obs_1, dim=1)
    ...
    def compute_rewards(self):
        ...
        self.rew_buf[:], self.reset_buf[:] = compute_exomy_reward(self.root_euler,
            ...
            self.heading_diff, self.target_dist, self.target_dist_diff, self.root_positions, 
            self.terrain_width_total, self.camera_penalty)        
...
def compute_exomy_reward(root_euler, reset_buf, progress_buf, max_episode_length, motor_velocities, 
    heading_diff, target_dist, target_dist_diff, root_positions, terrain_width_total, camera_penalty):
    # type: (Tensor, Tensor, Tensor, float, Tensor, Tensor, Tensor, Tensor, Tensor, float, Tensor) -> Tuple[Tensor, Tensor]
    ...
    # Penalty for tilting
    penaltyAngle = 0.4 #radians # absolut
    ...
    reset = torch.where(target_dist >= 7.0, ones, reset)
