...
class Exomy_simTest(VecTask):
    ...
    def _create_ground_plane(self):
        ...
        heightfield[0:num_rows, :] =            random_uniform_terrain(new_sub_terrain(), min_height=-0.015, max_height=0.015, step=0.05, downsampled_scale=0.5).height_field_raw
        heightfield[num_rows:2*num_rows, :] =   random_uniform_terrain(new_sub_terrain(), min_height=-0.015, max_height=0.015, step=0.05, downsampled_scale=0.5).height_field_raw
        heightfield[2*num_rows:3*num_rows, :] = random_uniform_terrain(new_sub_terrain(), min_height=-0.015, max_height=0.015, step=0.05, downsampled_scale=0.5).height_field_raw
        heightfield[3*num_rows:4*num_rows, :] = random_uniform_terrain(new_sub_terrain(), min_height=-0.015, max_height=0.015, step=0.05, downsampled_scale=0.5).height_field_raw
    ...
    def reset_idx(self, env_ids):
        ...
        for i in range(num_resets):
            r.append(R.from_euler('zyx', [torch.pi, 0, 0], degrees=False).as_quat())
    ...
    def post_physics_step(self):
        ...
        if self.target_dist[0] < 0.1:
            print("Target reached", self.progress_buf[0])
...
def compute_exomy_reward(root_euler, reset_buf, progress_buf, max_episode_length, motor_velocities, 
    ...
    reward = pos_reward + goal_reward + heading_reward + vel_penalty - distanceReset_penalty - time_penalty# - tilt_penalty
    ...
    reset = torch.where(target_dist < 0.001, ones, reset)
